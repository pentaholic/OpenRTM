...
//うまく行っていない。
  int flag_percent(int *pattern){
	...
	for(int i=0;i<20;i++){
		if(pattern[i]==0)		{n[0]++;	if(n[0]>=l){x=0;break;}
		}else if(pattern[i]==1)	{n[1]++;	if(n[1]>=l){x=1;break;}
		}else if(pattern[i]==2)	{n[2]++;	if(n[2]>=l){x=2;break;}
		}else if(pattern[i]==3)	{n[3]++;	if(n[3]>=l){x=3;break;}
		}else if(pattern[i]==4)	{n[4]++;	if(n[4]>=l){x=4;break;}
		}else if(pattern[i]==5)	{n[5]++;	if(n[5]>=l){x=5;break;}
		}else if(pattern[i]==6)	{n[6]++;	if(n[6]>=l){x=6;break;}
		}else if(pattern[i]==7)	{n[7]++;	if(n[7]>=l){x=7;break;}
		}else if(pattern[i]==8)	{n[8]++;	if(n[8]>=l){x=8;break;}
		}else if(pattern[i]==9)	{n[9]++;	if(n[9]>=l){x=9;break;}
		}else if(pattern[i]==10)	{n[10]++;	if(n[10]>=l){x=10;break;}
		}else if(pattern[i]==11)	{n[11]++;	if(n[11]>=l){x=11;break;}
		}else if(pattern[i]==12)	{n[12]++;	if(n[12]>=l){x=12;break;}
		}else if(pattern[i]==13)	{n[13]++;	if(n[13]>=l){x=13;break;}
		}else if(pattern[i]==14)	{n[14]++;	if(n[14]>=l){x=14;break;}
		}else if(pattern[i]==15)	{n[15]++;	if(n[15]>=l){x=15;break;}
		}else if(pattern[i]==16)	{n[16]++;	if(n[16]>=l){x=16;break;}
		}else if(pattern[i]==17)	{n[17]++;	if(n[17]>=l){x=17;break;}}
	}
...
//Z座標
  ...
  std::cout<<"フラッグパターン="<<fpat<<std::endl;
  ...
  if(num<1||num>16){//flagが基本姿勢or該当なしの場合、flagがパターン意外の場合
	pattern.push_back(fpat);//パターンをベクタ関数に代入
  }

  
  if(pattern.size()>19){//ベクタ関数patternが常に20個になるよう調整
  ...
  if(pattern.size()>=18){//20個ログが溜まったとき
	num=flag_percent(pattern.data());//フラッグがある％以上のときそのパターンを返す。ポインタ渡しが推奨だができていない。
	std::cout<<"可能性のあるパターンnum="<<num<<std::endl;

	if(num>=1&&num<=16){//○☓判定時に実行。基本姿勢と該当なしはなし
		std::cout<<"パターン"<<num<<"で合っていますか。○☓で判断してください。"<<std::endl;
