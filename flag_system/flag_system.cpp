...
//座標のチェック
  void printCoordinates(	double *xhead, double *yhead, double *zhead,
	...
	std::cout<<"首の座標x="		<<*xneck	<<", y="<<*yneck	<<", z="<<*zneck	<<std::endl;
	std::cout<<"右肩の座標x="	<<*r_xshoulder	<<", y="<<*r_yshoulder	<<", z="<<*r_zshoulder	<<std::endl;
	std::cout<<"右肘の座標x="	<<*r_xelbow	<<", y="<<*r_yelbow	<<", z="<<*r_zelbow	<<std::endl;
	std::cout<<"右手の座標x="	<<*r_xhand	<<", y="<<*r_yhand	<<", z="<<*r_zhand	<<std::endl;
	std::cout<<"左肩の座標x="	<<*l_xshoulder	<<", y="<<*l_yshoulder	<<", z="<<*l_zshoulder	<<std::endl;
	std::cout<<"左肘の座標x="	<<*l_xelbow	<<", y="<<*l_yelbow	<<", z="<<*l_zelbow	<<std::endl;
	std::cout<<"左手の座標x="	<<*l_xhand	<<", y="<<*l_yhand	<<", z="<<*l_zhand	<<std::endl;
  }

/*
//分散を求める
  double disparsion(double *disp){
	double sum;//和

  }

//分散のチェック
//10個ログを取ってそれの分散値を返却する
  int checkdispersion(double *xdisparsion,double *ydisparsion,double *zdisparsion,){
...
//うまく行っていない。
  int flag_percent(int *pattern){
	int n[20]={0};
	int x;
	
	int l=16;//16/20*100=80%以上でok

/*用済み
	//テスト用。パターンがしっかり受け渡しできているかチェック
	std::cout<<"flag_parcent内：pattern[i]のコピー"<<std::endl;
	for(int i=19;i>=0;i--)
...
*/
	for(int i=0;i<20;i++){
		for(int j=0;j<=17;j++){
			if(pattern[j]==j){
				n[j]++;if(n[j]>l){x=j;break;}
			};
		}
	}
	std::cout<<"flag_percent内：確定したx="<<x<<std::endl;//テスト用。

	return x;

  }

//○☓チェックのログを解析して○か☓かを返却する
  int oxcheck(int *circleorcross){
	int n[10]={0};
	int x;
	
	int l=8;//8/10*100=80%以上でok

	//テスト用。パターンがしっかり受け渡しできているかチェック
	/*用済み*/
	std::cout<<"oxcheck内：check[i]のコピー"<<std::endl;
	for(int i=9;i>=0;i--)
		std::cout<<"circleorcross["<<i<<"]="<<circleorcross[i]<<std::endl;
	

	for(int i=0;i<10;i++){
		for(int j=0;j<3;j++){
			if(circleorcross[j]==j){
				n[j]++;if(n[j]>l){x=j;break;}//1=○、0=☓,2=該当なし
			}
		}
	}
	return x;
...
//初めから○☓のジェスチャをしていると正しく認識する。参照渡しではないからエラーがでるのか！ポインタを使おう。そりゃコピーじゃダメだよね。
	while(1){
		...
		if(*r_xhand>*l_xhand//右手のx座標>左手のx座標
			...
			std::cout<<"OKorNG内:○☓プログラム、○です。"<<std::endl;
			x=1;
		//☓パターンok
...
//Z座標
  ...
  if(num<1||16<num){
	//フラッグパターンの判別
	int fpat=flag_pattern(xhead, yhead, zhead, xneck, yneck, zneck, r_xshoulder, r_xelbow, r_xhand, r_yshoulder, r_yelbow, r_yhand, r_zshoulder, r_zelbow,  r_zhand, l_xshoulder, l_xelbow, l_xhand, l_yshoulder, l_yelbow, l_yhand, l_zshoulder, l_zelbow, l_zhand);//パターンの選別

	std::cout<<"main内フラッグパターンfpat="<<fpat<<std::endl;


	//各座標の表示
	//printCoordinates(&xhead, &yhead, &zhead, &xneck, &yneck, &zneck, &r_xshoulder, &r_xelbow, &r_xhand, &r_yshoulder, &r_yelbow, &r_yhand, &r_zshoulder, &r_zelbow,  &r_zhand, &l_xshoulder, &l_xelbow, &l_xhand, &l_yshoulder, &l_yelbow, &l_yhand, &l_zshoulder, &l_zelbow, &l_zhand);//パターンの選別


	if(num<1||16<num){//flagが基本姿勢or該当なしの場合、flagがパターン以外の場合
		pattern.push_back(fpat);//パターンをベクタ関数に代入
	}


	//std::cout<<"main内:ログcircleorcrossのSize="<<circleorcross.size()<<std::endl;

	//patternログが常に20個になるよう調整
	if(pattern.size()>19){
		pattern.erase(pattern.begin());
	}

	std::cout<<"main内:ログpatternのSize="<<pattern.size()<<std::endl;

	//ベクタ関数patternの各項の値を表示。pattern.size()>0でないとコアダンプ。
	if(pattern.size()>0){
	...
	//20個ログが溜まったとき
	if(pattern.size()>=18){
		num=flag_percent(pattern.data());//フラッグがある％以上のときそのパターンを返す。
		std::cout<<"main内:20ログから得たパターンnum="<<num<<std::endl;
	}



  //numのパターンが確定したときの○☓判定を行う
  }else if(0<num&&num<16){

	//○☓判定時に実行。基本姿勢と該当なしはなし
	std::cout<<"main内パターン"<<num<<"で合っていますか。○☓で判断してください。"<<std::endl;

	//○☓の読み込み。
	int x=okorng(&xhead, &yhead, &zhead, &xneck, &yneck, &zneck, &r_xshoulder, &r_xelbow, &r_xhand, &r_yshoulder, &r_yelbow, &r_yhand, &r_zshoulder, &r_zelbow,  &r_zhand, &l_xshoulder, &l_xelbow, &l_xhand, &l_yshoulder, &l_yelbow, &l_yhand, &l_zshoulder, &l_zelbow, &l_zhand);//パターンの選別

	//○☓を○☓ログに格納
	circleorcross.push_back(x);

	//patternログが常に10個になるよう調整
	if(circleorcross.size()>10){
		circleorcross.erase(circleorcross.begin());
	}
	std::cout<<"circlecrossのサイズは"<<circleorcross.size()<<"です。"<<std::endl;
	
	//circleorcrossログの各項の値を表示。
	if(circleorcross.size()>0){
		for(int i=circleorcross.size();i>=0;i--)
			std::cout<<"circleorcross["<<i<<"]="<<circleorcross[i]<<std::endl;
	}
	...
	int c=2;//初期値：○☓以外
	if(circleorcross.size()==10)
		c=oxcheck(circleorcross.data());

	std::cout<<"main内:○(1)or☓(0)orなし(2)c="<<c<<std::endl;

	if(c==1){//○のとき
		std::cout<<"○を認識しました。"<<std::endl;
		std::cout<<"パターンは"<<num<<"です。"<<std::endl;
		pattern.clear();circleorcross.clear();num=0;
...
/*関数化して直上のif文に突っ込むべし。
	int a=3;	
	while(a!=0||a!=1){
		std::cout<<"入力してください。(終了[0]もう一回？[1])"<<std::endl;
		std::cin>>a;
