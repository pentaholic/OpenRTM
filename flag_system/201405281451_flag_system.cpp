...
//OKorNG20140527ここをやる。丸☓が認識するかどうかをまず確認しましょう。
...
//	std::cin>>x;
	std::cout<<"○ですか？☓ですか？"<<std::endl;
	
	while(1){
		//○パターンok
		if(r_xhand>l_xhand//右手のx座標>左手のx座標
			&&r_yhand>r_yelbow//右手のy座標>右肘のy座標
			&&l_yhand>l_yelbow//左手のy座標>左肘のy座標
...
//手旗のパターンを判断
  double flag_pattern(	double xhead, double yhead, double zhead,
			double xneck, double yneck, double zneck, 
			double r_xshoulder, double r_xelbow, double r_xhand,
			double r_yshoulder, double r_yelbow, double r_yhand,
			double r_zshoulder, double r_zelbow, double r_zhand,
...
//  std::cout<<"r_slope="	<<r_slope	<<"l_slope="	<<l_slope<<std::endl;
  ...
  n=flag_pattern(xhead, yhead, zhead, xneck, yneck, zneck, r_xshoulder, r_xelbow, r_xhand, r_yshoulder, r_yelbow, r_yhand, r_zshoulder, r_zelbow,  r_zhand, l_xshoulder, l_xelbow, l_xhand, l_yshoulder, l_yelbow, l_yhand, l_zshoulder, l_zelbow, l_zhand);//パターンの選別
  ...
  if(pattern.size()>0){//pattern.size()>0でないとコアダンプ
	...
	if(pattern.size()>18){//20個ログが溜まったとき
		std::cout<<n<<"で○☓判定してください。"<<std::endl;
		x=okorng(xhead, yhead, zhead, xneck, yneck, zneck, r_xshoulder, r_xelbow, r_xhand, r_yshoulder, r_yelbow, r_yhand, r_zshoulder,r_zelbow, r_zhand, l_xshoulder, l_xelbow, l_xhand, l_yshoulder, l_yelbow, l_yhand, l_zshoulder, l_zelbow, l_zhand);
